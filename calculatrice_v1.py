...
def page_theorie():
    ...
    axis_x = st.sidebar.selectbox("Axe X", NUMERIC_PARAMS, index=0,
                                  format_func=lambda k: PARAM_LABELS[k])
    axis_y = st.sidebar.selectbox("Axe Y", NUMERIC_PARAMS, index=1,
                                  format_func=lambda k: PARAM_LABELS[k])

    axis_z = None
    if mode == "Empilement 3D":
        axis_z = st.sidebar.selectbox("Axe Z (couches)", NUMERIC_PARAMS, index=2,
                                      format_func=lambda k: PARAM_LABELS[k])
    ...
    for p in NUMERIC_PARAMS:
        ...
        fixed_cfg[p] = st.sidebar.number_input(f"{PARAM_LABELS[p]} (fixe)",
                                               value=default_value)
    ...
    if mode == "Matrice 2D":
        ...
        # ---------- Bloc 4 images théorie/exp/comparaison ----------
        st.markdown("---")
        st.markdown("### Bloc 4 images théorie / expérience (points expérimentaux intégrés)")

        experimental_data = [
            {"V_kV": 30, "I_mA": 7,  "P_ubar": 20, "Dch_cm": 20, "Dgrid_cm": 5,
             "neutrons_exp": 1.0e4},
            {"V_kV": 40, "I_mA": 10, "P_ubar": 15, "Dch_cm": 20, "Dgrid_cm": 5,
             "neutrons_exp": 1.0e5},
            {"V_kV": 45, "I_mA": 12, "P_ubar": 12, "Dch_cm": 20, "Dgrid_cm": 5,
             "neutrons_exp": 2.0e5},
            {"V_kV": 50, "I_mA": 15, "P_ubar": 10, "Dch_cm": 20, "Dgrid_cm": 5,
             "neutrons_exp": 1.0e7},
        ]
        df_exp = pd.DataFrame(experimental_data)
        df_exp["flow_SCCM"] = 1.0
        df_exp["mat_grid"] = mat_grid
        df_exp["gas"] = gas
        ...
        for _, r in df_exp.iterrows():
            cfg2 = row_to_cfg2(r)
            Y_th_points.append(fusion_rate_DD(cfg2))
        df_exp["Y_th"] = Y_th_points

        eps = 1e-30
        df_exp["ratio"] = (df_exp["neutrons_exp"] + eps) / (df_exp["Y_th"] + eps)
        df_exp["log10_ratio"] = np.log10(df_exp["ratio"])

        # matrices sur (V_kV, I_mA)
        pivot_th = df_exp.pivot_table(
            index="I_mA", columns="V_kV", values="Y_th", aggfunc="mean"
        ).sort_index().sort_index(axis=1)
        pivot_exp = df_exp.pivot_table(
            index="I_mA", columns="V_kV", values="neutrons_exp", aggfunc="mean"
        ).sort_index().sort_index(axis=1)
        pivot_ratio = df_exp.pivot_table(
            index="I_mA", columns="V_kV", values="log10_ratio", aggfunc="mean"
        ).sort_index().sort_index(axis=1)
        ...
        def get_xyM(pivot):
            ...
            Mloc = pivot.values
            return x_vals, y_vals, Mloc

        x_th2, y_th2, M_th2 = get_xyM(pivot_th)
        x_exp2, y_exp2, M_exp2 = get_xyM(pivot_exp)
        x_r2, y_r2, M_r2 = get_xyM(pivot_ratio)

        fig_size = (3.6, 3.0)

        col1, col2 = st.columns(2)

        # 1) Matrice théorique (points exp)
        fig1, ax1 = plt.subplots(figsize=fig_size)
        im1 = ax1.imshow(M_th2, origin="lower", aspect="equal", interpolation="nearest")
        ax1.set_xlabel("V_kV")
        ax1.set_ylabel("I_mA")
        ax1.set_title("Matrice théorique $Y_{th}$ (points exp)", fontsize=11)
        ax1.set_xticks(np.arange(len(x_th2)))
        ax1.set_yticks(np.arange(len(y_th2)))
        ax1.set_xticklabels([f"{v:g}" for v in x_th2], rotation=90, fontsize=8)
        ax1.set_yticklabels([f"{v:g}" for v in y_th2], fontsize=8)
        ax1.set_xlim(-0.5, len(x_th2) - 0.5)
        ax1.set_ylim(-0.5, len(y_th2) - 0.5)
        ax1.grid(color="k", linewidth=0.2)
        fig1.subplots_adjust(left=0.18, right=0.88, top=0.88, bottom=0.20)
        fig1.colorbar(im1, ax=ax1, label=r"$Y_{\text{th}}$ (rel.)")
        with col1:
            st.markdown("**Théorie (points exp)**")
            st.pyplot(fig1)
        plt.close(fig1)

        # 2) Matrice expérimentale
        fig2, ax2 = plt.subplots(figsize=fig_size)
        im2 = ax2.imshow(M_exp2, origin="lower", aspect="equal", interpolation="nearest")
        ax2.set_xlabel("V_kV")
        ax2.set_ylabel("I_mA")
        ax2.set_title("Matrice expérimentale $Y_{exp}$", fontsize=11)
        ax2.set_xticks(np.arange(len(x_exp2)))
        ax2.set_yticks(np.arange(len(y_exp2)))
        ax2.set_xticklabels([f"{v:g}" for v in x_exp2], rotation=90, fontsize=8)
        ax2.set_yticklabels([f"{v:g}" for v in y_exp2], fontsize=8)
        ax2.set_xlim(-0.5, len(x_exp2) - 0.5)
        ax2.set_ylim(-0.5, len(y_exp2) - 0.5)
        ax2.grid(color="k", linewidth=0.2)
        fig2.subplots_adjust(left=0.18, right=0.88, top=0.88, bottom=0.20)
        fig2.colorbar(im2, ax=ax2, label=r"$Y_{\text{exp}}$ (n/s)")
        with col2:
        ...
        # 3) Matrice log10 ratio
        fig3, ax3 = plt.subplots(figsize=fig_size)
        im3 = ax3.imshow(M_r2, origin="lower", aspect="equal", interpolation="nearest")
        ax3.set_xlabel("V_kV")
        ax3.set_ylabel("I_mA")
        ax3.set_title(r"$\log_{10}(Y_{\text{exp}}/Y_{\text{th}})$", fontsize=11)
        ax3.set_xticks(np.arange(len(x_r2)))
        ax3.set_yticks(np.arange(len(y_r2)))
        ax3.set_xticklabels([f"{v:g}" for v in x_r2], rotation=90, fontsize=8)
        ax3.set_yticklabels([f"{v:g}" for v in y_r2], fontsize=8)
        ax3.set_xlim(-0.5, len(x_r2) - 0.5)
        ax3.set_ylim(-0.5, len(y_r2) - 0.5)
        ax3.grid(color="k", linewidth=0.2)
        fig3.subplots_adjust(left=0.18, right=0.88, top=0.88, bottom=0.20)
        fig3.colorbar(im3, ax=ax3,
                      label=r"$\log_{10}(Y_{\text{exp}}/Y_{\text{th}})$")
        with col3:
            st.markdown("**Comparaison**")
            st.pyplot(fig3)
        plt.close(fig3)

        # 4) Nuage de points exp vs th
        fig4, ax4 = plt.subplots(figsize=fig_size)
        Yth_plot = np.clip(df_exp["Y_th"].values, eps, None)
        Yexp_plot = np.clip(df_exp["neutrons_exp"].values, eps, None)
        ax4.scatter(Yth_plot, Yexp_plot, s=12)
        ax4.set_xscale("log")
        ax4.set_yscale("log")
        ax4.set_xlabel(r"$Y_{\text{th}}$ (modèle)")
        ax4.set_ylabel(r"$Y_{\text{exp}}$ (mesure)")
        ax4.set_title("Y_exp vs Y_th (log-log)", fontsize=11)
        y_min = min(Yth_plot.min(), Yexp_plot.min())
        y_max = max(Yth_plot.max(), Yexp_plot.max())
        ax4.plot([y_min, y_max], [y_min, y_max], "k--", linewidth=1)
        ax4.grid(True, which="both", linestyle=":")
        fig4.subplots_adjust(left=0.20, right=0.95, top=0.88, bottom=0.20)
        with col4:
            st.markdown("**Nuage exp/théorie**")
            st.pyplot(fig4)
        plt.close(fig4)
    ...
    # ----------------------------- MODE 3D -----------------------------
        ...
        x_vals, y_vals, z_vals, M = build_matrix_stack(axis_x, axis_y, axis_z,
                                                       ranges, fixed_cfg, qinfo["fn"])
        max_idx = np.unravel_index(np.argmax(M), M.shape)
...
def page_comparaison():
    ...
    st.dataframe(
        df[["V_kV", "I_mA", "P_ubar", "Dch_cm", "Dgrid_cm",
            "neutrons_exp", "Y_th", "log10_ratio"]]
    )
    ...
    x_th, y_th, M_th = get_xyM(pivot_th)
    x_exp, y_exp, M_exp = get_xyM(pivot_exp)
    x_r, y_r, M_r = get_xyM(pivot_ratio)
    ...
    # 1) Matrice théorique
    fig1, ax1 = plt.subplots(figsize=fig_size)
    im1 = ax1.imshow(M_th, origin="lower", aspect="equal", interpolation="nearest")
    ax1.set_xlabel(axis_x)
    ax1.set_ylabel(axis_y)
    ax1.set_title("Matrice théorique $Y_{th}$", fontsize=11)
    ax1.set_xticks(np.arange(len(x_th)))
    ax1.set_yticks(np.arange(len(y_th)))
    ax1.set_xticklabels([f"{v:g}" for v in x_th], rotation=90, fontsize=8)
    ax1.set_yticklabels([f"{v:g}" for v in y_th], fontsize=8)
    ax1.set_xlim(-0.5, len(x_th) - 0.5)
    ax1.set_ylim(-0.5, len(y_th) - 0.5)
    ax1.grid(color="k", linewidth=0.2)
    fig1.subplots_adjust(left=0.18, right=0.88, top=0.88, bottom=0.20)
    fig1.colorbar(im1, ax=ax1, label=r"$Y_{\text{th}}$ (rel.)")
    with col1:
    ...
    # 2) Matrice expérimentale
    fig2, ax2 = plt.subplots(figsize=fig_size)
    im2 = ax2.imshow(M_exp, origin="lower", aspect="equal", interpolation="nearest")
    ax2.set_xlabel(axis_x)
    ax2.set_ylabel(axis_y)
    ax2.set_title("Matrice expérimentale $Y_{exp}$", fontsize=11)
    ax2.set_xticks(np.arange(len(x_exp)))
    ax2.set_yticks(np.arange(len(y_exp)))
    ax2.set_xticklabels([f"{v:g}" for v in x_exp], rotation=90, fontsize=8)
    ax2.set_yticklabels([f"{v:g}" for v in y_exp], fontsize=8)
    ax2.set_xlim(-0.5, len(x_exp) - 0.5)
    ax2.set_ylim(-0.5, len(y_exp) - 0.5)
    ax2.grid(color="k", linewidth=0.2)
    fig2.subplots_adjust(left=0.18, right=0.88, top=0.88, bottom=0.20)
    fig2.colorbar(im2, ax=ax2, label=r"$Y_{\text{exp}}$ (n/s)")
    with col2:
    ...
    # 3) Matrice log10 ratio
    fig3, ax3 = plt.subplots(figsize=fig_size)
    im3 = ax3.imshow(M_r, origin="lower", aspect="equal", interpolation="nearest")
    ax3.set_xlabel(axis_x)
    ax3.set_ylabel(axis_y)
    ax3.set_title(r"$\log_{10}(Y_{\text{exp}}/Y_{\text{th}})$", fontsize=11)
    ax3.set_xticks(np.arange(len(x_r)))
    ax3.set_yticks(np.arange(len(y_r)))
    ax3.set_xticklabels([f"{v:g}" for v in x_r], rotation=90, fontsize=8)
    ax3.set_yticklabels([f"{v:g}" for v in y_r], fontsize=8)
    ax3.set_xlim(-0.5, len(x_r) - 0.5)
    ax3.set_ylim(-0.5, len(y_r) - 0.5)
    ax3.grid(color="k", linewidth=0.2)
    fig3.subplots_adjust(left=0.18, right=0.88, top=0.88, bottom=0.20)
    fig3.colorbar(im3, ax=ax3,
                  label=r"$\log_{10}(Y_{\text{exp}}/Y_{\text{th}})$")
    ...
    # 4) Nuage Y_exp vs Y_th
    fig4, ax4 = plt.subplots(figsize=fig_size)
    Yth_plot = np.clip(df["Y_th"].values, eps, None)
    Yexp_plot = np.clip(df["neutrons_exp"].values, eps, None)
    ax4.scatter(Yth_plot, Yexp_plot, s=12)
    ax4.set_xscale("log")
    ax4.set_yscale("log")
    ax4.set_xlabel(r"$Y_{\text{th}}$ (modèle)")
...
def main():
    ...
    page = st.sidebar.radio("Mode global",
                            ["Matrices théoriques", "Comparaison exp/théorie"])
